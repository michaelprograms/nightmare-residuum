...
void do_look_at_str(mixed args...) {
    ...
    if (!env->query_property("indoors") && member_array(
        str,
        ({ "sky", "sun" })
    ) > -1) {
        desc = D_ASTRONOMY->query_localsky(
            D_ASTRONOMY->query_astronomy_from_room(env),
            str
        );
    ...
    if (!desc) {
        ...
        message(
            "action",
            po->query_cap_name() + " looks over the " + str + ".",
            env,
            po
        );
    }
...
varargs mixed do_look_at_obj(object ob, mixed arg) {
    ...
    message(
        "action",
        po->query_cap_name() + " looks over " + ob->query_name() + ".",
        environment(ob),
        po
    );
...
mixed do_look_at_liv(object ob, mixed arg...) {
    ...
    message(
        "action",
        po->query_cap_name() + " looks over " + ob->query_cap_name() + ".",
        environment(ob),
        ({ po, ob })
    );
    ...
    if (po->query_immortal()) {
        ...
        message(
            "action",
            "Vitals: " + ob->query_hp() + "/" + ob->query_max_hp() + " " + ob->query_sp() + "/" + ob->query_max_sp() + " " + ob->query_mp() + "/" + ob->query_max_mp(),
            po
        );
    }
