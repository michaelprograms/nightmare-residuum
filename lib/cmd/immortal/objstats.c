...
void format_line(
    ...
    if (clones > -1) result += sprintf(
        "%5d  %5.2f",
        clones,
        (clones * 100.0 / totalAll)
    ) + "%   ";
    else result += "                ";
    if (leaks > -1) result += sprintf(
        "%5d  %5.2f",
        leaks,
        (leaks * 100.0 / totalAll)
    ) + "%   ";
    else result += "                ";
...
void command(string input, mapping flags) {
    ...
    int nObsLoaded = sizeof(filter(
        obs,
        (: $1 && !environment($1) && !regexp(file_name($1), "#") :)
    ));
    int nObsCloned = sizeof(filter(
        obs,
        (: $1 && environment($1) && regexp(file_name($1), "#") :)
    ));
    object *obsLeaked = filter(
        obs,
        (: $1 && !environment($1) && regexp(
            file_name($1),
            "#"
        ) && base_name($1) != "/std/user" :)
    );  // exclude login object
    int nLivsLoaded = sizeof(filter(
        livs,
        (: $1 && !environment($1) && !regexp(file_name($1), "#") :)
    ));
    int nLivsCloned = sizeof(filter(
        livs,
        (: $1 && environment($1) && regexp(file_name($1), "#") :)
    ));
    object *livsLeaked = filter(
        livs,
        (: $1 && !environment($1) && regexp(file_name($1), "#") :)
    );
    int nRoomsLoaded = sizeof(filter(
        rooms,
        (: $1 && !regexp(file_name($1), "#") :)
    ));
    ...
    format_line(
        "Total",
        nObsLoaded + nLivsLoaded + nRoomsLoaded,
        nObsCloned + nLivsCloned + nRoomsCloned,
        -1,
        totalObs + totalLivs + totalRooms,
        totalAll
    );
