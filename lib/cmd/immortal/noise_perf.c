...
void measure_perlin() {
    ...
    write(sprintf(
        ...
        "%10.6f",
        timeP / 1000000.0 / times
    ) + " ms%^RESET%^ x " + sprintf(
        "%d",
        times
    ) + " = %^ORANGE%^" + sprintf("%f", timeP / 1000000.0) + " ms%^RESET%^\n");
    write(sprintf(
        "%4s: ",
        "2D"
    ) + "%^ORANGE%^" + sprintf(
        "%10.6f",
        time2d / 1000000.0 / times
    ) + " ms%^RESET%^ x " + sprintf(
        "%d",
        times
    ) + " = %^ORANGE%^" + sprintf("%f", time2d / 1000000.0) + " ms%^RESET%^\n");
    write(sprintf(
        "%4s: ",
        "3D"
    ) + "%^ORANGE%^" + sprintf(
        "%10.6f",
        time3d / 1000000.0 / times
    ) + " ms%^RESET%^ x " + sprintf(
        "%d",
        times
    ) + " = %^ORANGE%^" + sprintf("%f", time3d / 1000000.0) + " ms%^RESET%^\n");
}
...
void measure_simplex() {
    ...
    for (int i = 0; i < times; i++) {
        ...
        pre = time_ns();
        total4d += "/secure/sefun/noise"->noise_simplex_4d(
            x,
            y,
            z,
            w,
