...
void command (string input, mapping flags) {
    ...
    styles = map(styles, (: ($1 != "-" ? "%^"+$1+"%^" : "")+sprintf("%15|s", $1)+" %^RESET%^" :));

    basicColors = map(basicColors, (: ($1 != "-" ? "%^"+$1+"%^" : "")+($1 == "BLACK" ? "B_WHITE%^" : "")+($1 == "B_WHITE" ? "BLACK%^" : "")+sprintf("%15|s", $1)+" %^RESET%^" :));
    ...
    xtermColors = map(xtermColors, (: ($1 != "-" ? "%^"+$1+"%^" : "")+(member_array($1, ({"AAA", "G01","G02","G03","G04","C16"})) > -1 ? "B_WHITE%^" : "")+sprintf("%10|s", $1)+" %^RESET%^" :));

    border(([
        "title": "COLORS",
        "header": ([
            "items": ({
                "You are set to use " + (this_user()->query_account()->query_setting("ansi") == "on" ? this_user()->query_terminal_color() : "no") + " colors",
            }),
            "columns": 1,
            "align": "center",
        ]),
