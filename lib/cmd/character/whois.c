...
void command(string input, mapping flags) {
    ...
    if (!char) {
        message(
            "action",
            "There is no character with that name on " + mud_name() + ".",
            tc
        );
        return;
    }
    ...
    headerItems += ({
        sprintf(
            "%12s",
            "Created"
        ) + ": " + strftime("%Y/%m/%u", char->query_created()),
    });
    if (curChar) {
        headerItems += ({
            sprintf(
                "%12s",
                "Idle"
            ) + ": " + time_from_seconds(time() - curChar->query_last_action()),
        });
    } else {
        headerItems += ({
            sprintf(
                "%12s",
                "Last on"
            ) + ": " + time_from_seconds(time() - char->query_last_action()),
        });
    }
