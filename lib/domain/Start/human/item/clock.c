...
void plot_circle(int xm, int ym, int r) {
    int x = -r, y = 0, err = 2 - 2 * r; /* II. Quadrant */ do {
        set_dot(
            ...
            xm + x,
            ym - y
        ); /* III. Quadrant */ set_dot(
            xm + y,
            ym + x
        ); /*  IV. Quadrant */ r = err;
        if (r > x) err += ++x * 2 + 1; /* e_xy+e_x > 0 */ if (r <= y) err += ++y * 2 + 1; /* e_xy+e_y < 0 */
    } while (x < 0);
}
...
string query_long() {
    ...
    localtime = D_ASTRONOMY->query_hour(
        t,
        a
    ) + ":" + D_ASTRONOMY->format_minute(D_ASTRONOMY->query_minute(
        t,
        a
    )) + ":" + D_ASTRONOMY->format_minute(D_ASTRONOMY->query_second(t, a));
