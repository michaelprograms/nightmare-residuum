...
void do_adopt_str(mixed args...) {
    ...
    pet->set_save_path(D_CHARACTER->query_save_path(
        po->query_key_name(),
        "pet"
    ));
    pet->set_owner(po);
    ...
    message(
        "action",
        "A squirrel walks over to " + possessive_noun(po->query_cap_name()) + " side.",
        environment(po),
        po
    );
...
void do_reclaim_str(mixed args...) {
    ...
    message(
        "action",
        possessive_noun(po->query_cap_name()) + " squirrel walks over to " + possessive(po) + " side.",
        environment(po),
        po
    );
    pet->handle_move(environment(po));
