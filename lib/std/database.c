...
mixed query_handle () { return dbHandle; }

/* -----  ----- */

mixed connect (mapping config) {
    if (!mapp(config)) error("Bad argument 1 to database->connect");
    if (dbHandle) error("Database already connected");

    if (stringp(config["host"])) dbHost = config["host"];
    if (undefinedp(config["db"])) error("Bad argument 1 to database->connect: missing db");
    dbDatabase = config["db"];
    if (config["user"]) dbUser = config["user"];
    if (config["type"]) dbType = config["type"];
    if (config["autoclose"]) dbAutoclose = config["autoclose"];

    dbHandle = db_connect(dbHost, dbDatabase, dbUser, dbType);
    if (stringp(dbHandle)) error(dbHandle);
    return this_object();
}
...
mixed query (string sql) {
    ...
    if (!dbHandle) error("Database not connected");
