...
void test_history() {
    ...
    expect("history catches bad inputs", (: ({
        assert_catch(
            (: testOb->add_tell_history(0) :),
            "*Bad argument 1 to history->add_tell_history\n"
        ),

        testOb->add_tell_history("Test1"),
        assert_catch(
            (: testOb->add_say_history(0) :),
            "*Bad argument 1 to history->add_say_history\n"
        ),
    }) :));
}
