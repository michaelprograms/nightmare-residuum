...
void test_achievements() {
    ...
    expect("achievements are settable and queryable", (: ({
        ...
        assert_equal(
            testOb->set_achievement_flag("test1", "2", "1,2,3"),
            ([ "done": 0, "flags": "2" ])
        ),
        assert_equal(
            testOb->query_achievement("test1"),
            ([ "done": 0, "flags": "2" ])
        ),
        assert_equal(
            testOb->set_achievement_flag("test1", "2", "1,2,3"),
        ...
        assert_equal(
            testOb->set_achievement_flag("test1", "3", "1,2,3"),
            ([ "done": 0, "flags": "2,3" ])
        ),
        assert_equal(
            testOb->set_achievement_flag("test1", "1", "1,2,3"),
            ([ "done": 1 ])
        ),
        assert_equal(testOb->query_achievement("test1"), ([ "done": 1 ])),

        assert_equal(
            testOb->set_achievement_flag("test2", "1", "1"),
            ([ "done": 1 ])
        ),
        assert_equal(testOb->query_achievement("test2"), ([ "done": 1 ])),
        assert_equal(
            testOb->query_achievements_done(),
            ({ "test2", "test1", })
        ),
