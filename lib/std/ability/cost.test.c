...
void test_cost () {
    ...
    expect("handles query_cost", (: ({
        ...
        $(mockConfig)->set_powers(([ "ranged": 10 ])),
        assert_equal(testOb->query_cost(), ([ "sp": 10, "mp": 0 ])),

        $(mockConfig)->set_powers(([ "brawl": 20 ])),
        assert_equal(testOb->query_cost(), ([ "sp": 20, "mp": 0 ])),

        $(mockConfig)->set_powers(([ "psionic": 123 ])),
        assert_equal(testOb->query_cost(), ([ "sp": 0, "mp": 123 ])),

        $(mockConfig)->set_powers(([ "brawl": 31, "psionic": 32 ])),
    ...
    expect("handles verify_cost", (: ({
        ...
        assert_equal(testOb->verify_cost(this_object(), $(cost)), 0),
        ...
        assert_equal(testOb->verify_cost(this_object(), $(cost)), 1),
    }) :));

    expect("handles apply_cost", (: ({
        testOb->apply_cost(this_object(), $(cost)),
