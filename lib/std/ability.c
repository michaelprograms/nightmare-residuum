...
int calculate_damage (object source, object target, string type) {
    int damage;
    ...
    damage += secure_random(source->query_skill(type + " attack") * 20 / 100 + 1);
    ...
    damage -= secure_random(target->query_skill(type + " defense") * 20 / 100 + 1);
