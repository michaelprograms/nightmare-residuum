...
void test_user () {
    ...
    expect("user is settable and queryable", (: ({
        ...
        assert(testOb->query_user(), "==", this_object()),
        testOb->set_user(0),
        assert(testOb->query_user(), "==", 0),
    }) :));
}
...
void test_times () {
    ...
    expect("user has a created time", (: ({
        ...
        assert(testOb->query_created(), "==", time()),
    }) :));
    ...
    expect("user has a connection time", (: ({
        // last action time is now
        assert(testOb->query_connection_time(), "==", 0),
    }) :));
}
...
void test_name () {
    ...
    expect("name handles character setup", (: ({
        ...
        assert(testOb->query_name(), "==", "testcharacter"),
...
void test_immortal () {
    ...
    expect("immortal is settable and queryable", (: ({
        assert(testOb->query_immortal(), "==", UNDEFINED),
        ...
        assert(testOb->query_immortal(), "==", 0),
        assert(unguarded((: file_size("/realm/testcharacter") :)), "==", -1),

        // verify turning on
        testOb->set_immortal(1),
        assert(testOb->query_immortal(), "==", 1),
        assert(unguarded((: file_size("/realm/testcharacter") :)), "==", -2),
        ...
        assert(unguarded((: rmdir("/realm/testcharacter") :)), "==", 1),
    }) :));
}