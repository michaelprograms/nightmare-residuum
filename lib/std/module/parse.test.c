...
void test_add_rules_with_mock() {
    ...
    expect("rules and synonyms are added", (: ({
        ...
        assert_equal(
            $(mockParse)->query_rules(),
            ({ ({ "module/parse", "" }), ({ "module/parse", "LVS" }) })
        ),
        assert_equal(
            $(mockParse)->query_synonym(),
            ({
                ({ "synonym1", "module/parse", "" }),
                ({ "synonym2", "module/parse", "" }),
                ({ "synonym1", "module/parse", "LVS" }),
                ({ "synonym2", "module/parse", "LVS" })
            })
        ),
