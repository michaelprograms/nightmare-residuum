...
void test_currencies() {
    ...
    expect("currency handles bad inputs", (: ({
        assert_catch(
            (: testOb->query_currency(UNDEFINED) :),
            "*Bad argument 1 to currency->query_currency\n"
        ),

        assert_catch(
            (: testOb->add_currency(UNDEFINED, UNDEFINED) :),
            "*Bad argument 1 to currency->add_currency\n"
        ),
        assert_catch(
            (: testOb->add_currency("copper", UNDEFINED) :),
            "*Bad argument 2 to currency->add_currency\n"
        ),
    }) :));
}
