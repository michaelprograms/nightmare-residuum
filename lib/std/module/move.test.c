...
void test_environment() {
    expect("handles setting and querying environment path and short", (: ({
        ...
        assert_equal(
            testOb->query_environment_path(),
            "/domain/Nowhere/room/freezer.c"
        ),
        assert_equal(testOb->query_environment_short(), "a freezer"),

        // use nonexistent path
        assert_catch(
            (: testOb->set_environment_path("/nonexistent.c") :),
            "*Bad argument 1 to move->set_environment_path\n"
        ),
        assert_equal(testOb->query_dest_ob("/nonexistent.c"), 0),
        assert_catch(
            (: testOb->handle_move("/nonexistent.c") :),
            "*Bad argument 1 to move->handle_move\n"
        ),
        ...
        assert_equal(
            testOb->query_environment_short(),
            "move.test.c query_short"
        ),
