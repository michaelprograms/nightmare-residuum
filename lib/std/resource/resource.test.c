...
void test_set_type() {
    expect("set_type rejects unknown types", (: ({
        assert_catch(
            ...
            "*Bad argument 1 to resource->set_type\n"
        ),
        assert_catch(
            (: testOb->set_type(0) :),
            "*Bad argument 1 to resource->set_type\n"
...
void test_descriptions() {
    expect("set_type and set_level name the resource", (: ({
        ...
        assert_equal(
            testOb->query_long(),
            "A chunk containing a strip of copper ore."
        ),
        ...
        assert_equal(
            testOb->query_long(),
            "A ragged hide stripped from a creature."
        ),
        ...
        assert_equal(
            testOb->query_long(),
            "A piece of alloy salvaged from a structure."
        ),
