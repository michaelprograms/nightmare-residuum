...
void test_species() {
    expect("species settable and queryable", (: ({
        ...
        assert_catch(
            (: testOb->set_species(UNDEFINED) :),
            "*Bad argument 1 to body->set_species\n"
        ),
    }) :));
}
...
void test_limbs() {
    expect("species limbs are queryable", (: ({
        ...
        assert_equal(
            sort_array(testOb->query_limbs(), 1),
            ({
                "head",
                "left arm",
                "left foot",
...
void test_sever_and_restore() {
    ...
    expect("limbs are severable", (: ({
        ...
        assert_equal(
            ...
            "You are already wielding weapon1."
        ),
...
void test_limbs_and_level() {
    ...
    expect("setting species first has maxdamage", (: ({
        $(mockBody)->set_stat("endurance", 20),
        testOb->set_species("human"),
    ...
    expect("maxdamage to increase with endurance", (: ({
        ...
        $(mockBody)->set_stat("endurance", 40),
        assert_equal(
            testOb->query_limb("torso"),
            ([
                "damage": 0,
...
void test_injections() {
    ...
    expect("injections are addable and queryable", (: ({
        ...
        assert_equal(
            testOb->query_injections(),
            ([ "healing nanites": 10, "damaging nanites": 5, ])
        ),
        ...
        assert_equal(
            testOb->query_injections(),
            ([ "healing nanites": 10, "damaging nanites": 10, ])
        ),
    ...
    expect("injections are handled by heart_beat", (: ({
        ...
        assert_equal(
            testOb->query_injections(),
            ([ "healing nanites": 10, "damaging nanites": 8, ])
        ),
        assert_equal(testOb->query_injection("healing nanites"), 10),
        ...
        assert_equal(
            testOb->query_injections(),
            ([ "healing nanites": 10, "damaging nanites": 6, ])
        ),
        assert_equal(testOb->query_injection("healing nanites"), 10),
...
void test_heal() {
    ...
    expect("heal restores hp/sp/mp and limb damage", (: ({
        ...
        assert_equal($(mockBody)->query_hp(), 22),
        assert_equal($(mockBody)->query_sp(), 12),
        assert_equal($(mockBody)->query_mp(), 12),
        assert_equal(
            testOb->query_limb("torso"),
            ([
                "damage": 0,
        ...
        testOb->heal(10),
        assert_equal($(mockBody)->query_hp(), 11),
        assert_equal($(mockBody)->query_sp(), 11),
        assert_equal($(mockBody)->query_mp(), 11),
        assert_equal(
            testOb->query_limb("torso"),
            ([
                "damage": 15,
...
void test_armor_and_weapons() {
    ...
    expect("querying armor behaves", (: ({
        ...
        assert_equal(
            testOb->handle_unwear($(armor1)),
            "You are not wearing armor1."
        ),
        ...
        assert_equal(
            testOb->handle_wear($(armor2)),
            "You cannot wear a shield while wielding a weapon in your left hand."
        ),

        $(weapon2)->set_type("sword"),
        $(weapon2)->set_hands(1),
        assert_equal(
            testOb->handle_wield($(weapon2)),
            "You are out of limbs to wield weapon2."
        ),
        ...
        assert_equal(
            testOb->handle_wear($(armor1)),
            "You are already wearing armor1."
        ),
        assert_equal(
            testOb->handle_wear($(armor2)),
        ...
        assert_equal(
            testOb->handle_unwield($(weapon1)),
            "You are not wielding weapon1."
        ),
