...
void test_cooldown_setup() {
    ...
    expect("cooldown setup errors are caught", (: ({
        assert_catch(
            (: testOb->set_cooldown(UNDEFINED) :),
            "*Bad argument 1 to cooldown->set_cooldown\n"
        ),
        assert_catch(
            (: testOb->set_cooldown("test", 12.3, "heart_beat") :),
            "*Bad argument 2 to cooldown->set_cooldown\n"
        ),
        assert_catch(
            (: testOb->set_cooldown("test", 12, "timed") :),
            "*Bad argument 2 to cooldown->set_cooldown\n"
        ),
        assert_catch(
            (: testOb->set_cooldown("test", 12, "invalid") :),
            "*Bad argument 3 to cooldown->set_cooldown\n"
        ),
    }) :));
}
...
void test_cooldown_heart_beats() {
    expect("heart_beat cooldowns are settable and queryable", (: ({
        ...
        assert_equal(
            testOb->query_cooldowns(),
            ([ "test": ([ "type": "heart_beat", "value": 1 ]) ])
        ),
