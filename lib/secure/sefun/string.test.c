...
void test_identify() {
    ...
    expect("identify handles function", (: ({
        ...
        assert_equal(
            ...
            "(: call_other, " + file_name() + ", \"query_name\" :)"
        ),
    ...
    expect("identify handles class", (: ({
        ...
        assert_equal(
            testOb->identify(tc),
            "CLASS( 6 elements  \"test string\",  ({ /* sizeof() == 2 */    \"test string 1\",    \"test string 2\"  }),  123,  1.000000,  ([ /* sizeof() == 1 */    \"test\" : 123,  ]),  " + file_name() + " )"
        ),
    }) :));
    ...
    expect("identify handles buffer", (: ({
        assert_equal(
            testOb->identify(read_buffer("/secure/sefun/string.test.c", 0, 8)),
            "BUFFER (0x69, 0x6E, 0x68, 0x65, 0x72, 0x69, 0x74, 0x20)"
        ),
    }) :));
}
...
void test_wrap() {
    ...
    expect("wrap handles wrapping text", (: ({
        ...
        assert_equal(
            testOb->wrap("testtesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttest"),
            "testtesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttest" + $(resetANSI) + $(linewrap) + "test"
        ),
        assert_equal(
            testOb->wrap("testtest", 4),
            "test" + $(resetANSI) + $(linewrap) + "test"
        ),
        assert_equal(
            testOb->wrap("testtesttest", 10),
            "testtestte" + $(resetANSI) + $(linewrap) + "st"
        ),
        assert_equal(
            testOb->wrap("testtesttest", 10, 2),
            "testtestte" + $(resetANSI) + $(linewrap) + "  st"
        ),
        ...
        assert_equal(
            testOb->wrap("testtesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttest"),
            "testtesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttest" + $(linewrap) + "test"
        ),
        assert_equal(
            testOb->wrap("testtest", 4),
            "test" + $(linewrap) + "test"
        ...
        assert_equal(
            testOb->wrap(
                "%^RED%^testtesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttesttest%^RESET%^",
                40,
                4
            ),
...
void test_sanitize_name() {
    ...
    expect("sanitize_name handles bad input", (: ({
        assert_catch(
            (: testOb->sanitize_name(UNDEFINED) :),
            "*Bad argument 1 to string->sanitize_name\n"
        ),
        assert_catch(
            (: testOb->sanitize_name(0) :),
            "*Bad argument 1 to string->sanitize_name\n"
        ),
    }) :));
}
...
void test_parse_command_flags() {
    expect("parse_command_flags parses valid flags", (: ({
        // check no flags
        assert_equal(
            testOb->parse_command_flags("something"),
            ({ "something", ([]) })
        ),
        ...
        assert_equal(
            testOb->parse_command_flags("-a something"),
            ({ "something", ([ "a": 1, ]) })
        ),
        assert_equal(
            testOb->parse_command_flags("-abc something"),
            ({ "something", ([ "abc": 1, ]) })
        ),
        assert_equal(
            testOb->parse_command_flags("-a -b -c something"),
            ({ "something", ([ "a": 1, "b": 1, "c": 1, ]) })
        ),
        assert_equal(
            testOb->parse_command_flags("-a=1 -b=2 -c=3 something"),
            ({ "something", ([ "a": "1", "b": "2", "c": "3", ]) })
        ),
        assert_equal(
            testOb->parse_command_flags("-a=x -b=y -c=z something"),
            ({ "something", ([ "a": "x", "b": "y", "c": "z", ]) })
        ),
        assert_equal(
            testOb->parse_command_flags("-path=/path/with/special_characters-in.it something else"),
            ({
                "something else",
                ([ "path": "/path/with/special_characters-in.it", ])
            })
        ),

        // check flags with no trailing input
        assert_equal(
            testOb->parse_command_flags("-a"),
            ({ "", ([ "a": 1, ]) })
        ),
        assert_equal(
            testOb->parse_command_flags("-a -b"),
            ({ "", ([ "a": 1, "b": 1, ]) })
        ),
    ...
    expect("parse_command_flags parses invalid flags", (: ({
        ...
        assert_equal(
            testOb->parse_command_flags("something -invalidflag"),
            ({ "something -invalidflag", ([]) })
        ),
        assert_equal(
            testOb->parse_command_flags("x y z -a"),
            ({ "x y z -a", ([]) })
        ),
        assert_equal(
            testOb->parse_command_flags("x y z -abc"),
            ({ "x y z -abc", ([]) })
        ),
    }) :));
}
