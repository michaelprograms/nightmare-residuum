...
string *split_path (string path) {
    int pos;
    while (path[<1] == '/' && sizeof(path) > 1) path = path[0..<2];
    pos = strstr(path, "/", -1);
...
string sanitize_path (string path) {
    ...
    if (!path || path == "" || (path[0] != '/' && path[0] != '~' && path[0] != '^')) {
    ...
    if (path[0] == '^') {
        path = regreplace(path, "^", "domain/", 1);
    } else if (path[0] == '~') {
        path = regreplace(path, "~", "realm/" + SEFUN->this_character()->query_key_name() + "/", 1);
    }
    parts = explode(path, "/") - ({ "", "." });
    if (!sizeof(parts)) {
        return "/";
...
varargs string absolute_path (string relative_path, mixed relative_to) {
    ...
    if (relative_path && relative_path[0] != '/' && relative_path[0] != '^' && relative_path[0] != '~') {
        if (objectp(relative_to)) {
            relative_path = base_path(program_name(relative_to)[0..<3]) + "/" + relative_path;
...
int assure_dir (string path) {
    ...
    dirs = explode(dirs[0], "/") + (!sizeof(regexp(({ dirs[1] }), "\\.")) ? ({ dirs[1] }) : ({}));
...
string *wild_card (string path, string relative_to) {
    ...
    for (int i = 0; i < sizeof(match); i ++) {
        ...
        if (sizeof(match[i]) > 1 && match[i][0..1] == "//") {
            match[i] = match[i][1..<1];
        }
    }
    return match;
