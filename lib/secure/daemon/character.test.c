...
void test_save_path() {
    expect("query_save_path handles valid names", (: ({
        assert_regex(
            testOb->query_save_path("name", "character"),
            "^/save/character/n/name/character"
        ),
        assert_regex(
            testOb->query_save_path("name", "pet"),
            "^/save/character/n/name/pet"
        ),
        assert_regex(
            testOb->query_save_path("tester"),
            "^/save/character/t/tester/character.o"
        ),
    }) :));
    ...
    expect("query_save_path handles invalid names", (: ({
        ...
        assert_equal(
            testOb->query_save_path("nonononononononononononononononononononononononononononononononom"),
            0
        ),
...
void test_exists() {
    expect("query_exists handles invalid characters", (: ({
        ...
        assert_equal(
            testOb->query_exists("testcharacterinvalid", "character"),
            0
        ),
    }) :));
    ...
    write_file(
        "/save/character/c/charactertest/character.o",
        "charactertest.o mock save",
        1
    );
...
void test_query_character() {
    ...
    expect("query_character returns loaded character", (: ({
        assert_regex(
            __TestCharacter = testOb->query_character("charactertest"),
            "/std/character#"
        ),
    }) :));
