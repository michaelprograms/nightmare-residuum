...
void test_valid_name() {
    expect("query_valid_name handles invalid names", (: ({
        ...
        assert_equal(
            testOb->query_valid_name("nonononononononononononononononononononononononononononononononom"),
            0
        ),
        assert_equal(testOb->query_valid_name("invalid!@#"), 0),
        assert_equal(testOb->query_valid_name("INVALID!@#"), 0),
        assert_equal(
            testOb->query_valid_name(",<.>;:'\"[{]}\\|=+!#$%^&*()~`"),
            0
        ),
...
void test_exists() {
    ...
    expect("query_exists handles valid accounts", (: ({
        ...
        assert_equal(
            write_file(
                "/save/account/a/accounttest.o",
                "accounttest.o mock save",
                1
            ),
            1
        ),
