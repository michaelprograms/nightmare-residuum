...
int query_allowed (object caller, string fn, string file, string mode) {
    ...
    if (__Unguarded == caller) {
        ...
        // access check passes due to character requesting matching save path
        if (!strsrch(tmp, "/std/character") && D_CHARACTER->query_save_path(caller->query_key_name()) == file) return 1;
        else if (!strsrch(tmp, "/std/account") && D_ACCOUNT->query_save_path(caller->query_key_name()) == file) return 1;
        else {
