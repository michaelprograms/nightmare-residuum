...
void test_query_file_privs() {
    expect("query_file_privs returns correct priv for each path class", (: ({
        ...
        assert_equal(
            testOb->query_file_privs("/cmd/character/look.c"),
            ACCESS_CMD
        ),
        assert_equal(testOb->query_file_privs("/daemon/ansi.c"), ACCESS_MUDLIB),
        assert_equal(
            testOb->query_file_privs("/daemon/something.c"),
            ACCESS_MUDLIB
        ),
        assert_equal(testOb->query_file_privs("/std/object.c"), ACCESS_ASSIST),
        assert_equal(
            testOb->query_file_privs("/std/module/save.c"),
            ACCESS_ASSIST
        ),
        assert_equal(
            testOb->query_file_privs("/save/account/test"),
            ACCESS_SECURE
        ),
        assert_equal(
            testOb->query_file_privs("/secure/daemon/access.c"),
            ACCESS_SECURE
        ),
        assert_equal(
            testOb->query_file_privs("/secure/daemon/master.c"),
            ACCESS_SECURE
        ),
        assert_equal(
            testOb->query_file_privs("/realm/user/workroom.c"),
            "user"
        ),
        assert_equal(testOb->query_file_privs("/realm/user/header.h"), "user"),
        assert_equal(
            testOb->query_file_privs("/domain/Area/room/room1.c"),
            "Area"
        ),
        assert_equal(
            testOb->query_file_privs("/domain/Area/include/area.h"),
            "Area"
        ),
        assert_equal(
            testOb->query_file_privs("/domain/CAPITALIZED/file.c"),
            "Capitalized"
        ),
...
void test_query_allowed_reads() {
    ...
    expect("query_allowed allows reads from ALL paths for any caller", (: ({
        ...
        assert_equal(
            testOb->query_allowed(testOb, "file_size", "/tmp/path", "read"),
            1
        ),
        assert_equal(
            testOb->query_allowed($(basicOb), "file_size", "/tmp/path", "read"),
            1
        ),
...
void test_query_allowed_writes() {
    ...
    expect("query_allowed allows writes to ALL paths for any caller", (: ({
        ...
        assert_equal(
            testOb->query_allowed(testOb, "write_file", "/tmp/path", "write"),
            1
        ),
        assert_equal(
            testOb->query_allowed(
...
void test_query_allowed_socket() {
    expect("query_allowed denies socket access to non-IPC callers", (: ({
        ...
        assert_equal(
            testOb->query_allowed(testOb, "socket_create", 0, "socket"),
            0
        ),
    }) :));
}
...
void test_query_allowed_invalid_args() {
    expect("query_allowed rejects invalid arguments", (: ({
        assert_catch((: testOb->query_allowed(
            0,
            "file_size",
            "/tmp/path",
            "read"
