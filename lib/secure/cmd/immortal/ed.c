...
void display(object tc, object target, string type, string old, string now) {
    if (tc == target) {
        message(
            "action",
            "You have changed your " + type + " from " + old + " to " + now + ".",
            tc
        );
    } else {
        message(
            "action",
            "You have changed " + possessive_noun(target) + " " + type + " from " + old + " to " + now + ".",
            tc
        );
        message(
            "action",
            tc->query_cap_name() + " has changed your " + type + " from " + old + " to " + now + ".",
            target
        );
    }
}
...
void command(string input, mapping flags) {
    ...
    if (!input && !sizeof(flags)) {
        message(
            "action",
            "Syntax: evolve (-c=class) (-l=[1+]) (-s=[1-100]) ([target])",
            tc
        );
        return;
    }
    ...
    foreach (string stat in ({
        ...
        statMax = D_CLASS->query_max_stat(
            target->query_class(),
            stat,
            target->query_level()
        );
