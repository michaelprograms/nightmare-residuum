...
void test_send() {
    ...
    expect("channels can send", (: ({
        ...
        testOb->send(
            "connection",
            this_object(),
            "Not allowed to send messages on this line."
        ),
    ...
    expect("channels toggle to blocked", (: ({
        ...
        assert_equal(blocked["chat"], 0),
        assert_equal(
            testOb->query_history("chat")[<1],
            "ChannelTest [[chat]] New message."
        ),
    }) :));
...
void test_send_system() {
    ...
    expect("system channels can send", (: ({
        ...
        assert_equal(
            testOb->query_history("error"),
            ({ "((error)) Test error." })
        ),

        testOb->send_system("connection", "Player status change."),
        assert_equal(
            testOb->query_history("connection"),
            ({ "((connection)) Player status change." })
        ),
