...
void test_localdate() {
    expect("handles localdate of an almanac", (: ({
        assert_equal(
            testOb->query_localdate(__Almanac, __Time + (DAY * 0)),
            "1 of Roki 1"
        ),
        assert_equal(
            testOb->query_localdate(__Almanac, __Time + (DAY * 19)),
            "20 of Roki 1"
        ),
        assert_equal(
            testOb->query_localdate(__Almanac, __Time + (DAY * 20)),
            "1 of Praxi 1"
        ),
        assert_equal(
            testOb->query_localdate(__Almanac, __Time + (DAY * 100)),
            "1 of Kepki 1"
        ),
        assert_equal(
            testOb->query_localdate(__Almanac, __Time + (DAY * 200)),
            "1 of Roki 2"
        ),
        assert_equal(
...
void test_day_of_year() {
    expect("day of year stays within bounds", (: ({
        assert_equal(
            testOb->query_day_of_year(__Time + (DAY * 0), __Almanac),
            0
        ),
        assert_equal(
            testOb->query_day_of_year(__Time + (DAY * 1), __Almanac),
            1
        ),
        assert_equal(
            testOb->query_day_of_year(__Time + (DAY * 25), __Almanac),
            25
        ),
        assert_equal(
            testOb->query_day_of_year(__Time + (DAY * 50), __Almanac),
            50
        ),
        assert_equal(
            testOb->query_day_of_year(__Time + (DAY * 75), __Almanac),
            75
        ),
        assert_equal(
            testOb->query_day_of_year(__Time + (DAY * 100), __Almanac),
            100
        ),
        assert_equal(
            testOb->query_day_of_year(__Time + (DAY * 199), __Almanac),
            199
        ),
        assert_equal(
            testOb->query_day_of_year(__Time + (DAY * 200), __Almanac),
            0
        ),
    }) :));
}
...
void test_almanac() {
    expect("handles dawn and dusk throughout year", (: ({
        assert_equal(
            testOb->query_calculate_almanac(__Time + (DAY * 0), __Almanac),
            ([
                "dawn": ({ 4, 0 }),
                "day": ({ 5, 0 }),
                "dusk": ({ 16, 0 }),
        ...
        assert_equal(
            testOb->query_calculate_almanac(__Time + (DAY * 25), __Almanac),
            ([
                "dawn": ({ 4, 30 }),
                "day": ({ 5, 30 }),
                "dusk": ({ 15, 30 }),
        ...
        assert_equal(
            testOb->query_calculate_almanac(__Time + (DAY * 50), __Almanac),
            ([
                "dawn": ({ 5, 0 }),
                "day": ({ 6, 0 }),
                "dusk": ({ 15, 0 }),
        ...
        assert_equal(
            testOb->query_calculate_almanac(__Time + (DAY * 75), __Almanac),
            ([
                "dawn": ({ 5, 30 }),
                "day": ({ 6, 30 }),
                "dusk": ({ 14, 30 }),
        ...
        assert_equal(
            testOb->query_calculate_almanac(__Time + (DAY * 100), __Almanac),
            ([
                "dawn": ({ 6, 0 }),
                "day": ({ 7, 0 }),
                "dusk": ({ 14, 0 }),
        ...
        assert_equal(
            testOb->query_calculate_almanac(__Time + (DAY * 125), __Almanac),
            ([
                "dawn": ({ 5, 30 }),
                "day": ({ 6, 30 }),
                "dusk": ({ 14, 30 }),
        ...
        assert_equal(
            testOb->query_calculate_almanac(__Time + (DAY * 150), __Almanac),
            ([
                "dawn": ({ 5, 0 }),
                "day": ({ 6, 0 }),
                "dusk": ({ 15, 0 }),
        ...
        assert_equal(
            testOb->query_calculate_almanac(__Time + (DAY * 175), __Almanac),
            ([
                "dawn": ({ 4, 30 }),
                "day": ({ 5, 30 }),
                "dusk": ({ 15, 30 }),
        ...
        assert_equal(
            testOb->query_calculate_almanac(__Time + (DAY * 200), __Almanac),
            ([
                "dawn": ({ 4, 0 }),
                "day": ({ 5, 0 }),
                "dusk": ({ 16, 0 }),
...
void test_day_and_month_names() {
    expect("day and month names track the calendar", (: ({
        ...
        assert_equal(
            testOb->query_day_name(__Time + (DAY * 1), __Almanac),
            "Lockday"
        ),
        assert_equal(
            testOb->query_day_name(__Time + (DAY * 4), __Almanac),
            "Vaigday"
        ),

        assert_equal(testOb->query_month_name(__Time, __Almanac), "Roki"),
        assert_equal(
            testOb->query_month_name(__Time + (DAY * 20), __Almanac),
            "Praxi"
        ),
        assert_equal(
            testOb->query_month_name(__Time + (DAY * 180), __Almanac),
            "Denki"
        ),
    }) :));
}
...
void test_localsky() {
    expect("localsky describes the sky per day phase", (: ({
        ...
        assert_equal(
            testOb->query_localsky(__Almanac, "sky"),
            "%^ORANGE%^BOLD%^It is lit with the colors of a brand new day.%^RESET%^"
        ),
        (__Almanac["DAY_PHASE"] = "day"),
        assert_equal(
            testOb->query_localsky(__Almanac, "sky"),
            "%^I_YELLOW%^BOLD%^The sun lights up the daytime sky.%^RESET%^"
        ),
        (__Almanac["DAY_PHASE"] = "dusk"),
        assert_equal(
            testOb->query_localsky(__Almanac, "sky"),
            "%^CYAN%^BOLD%^The sun is fading over the western horizon.%^RESET%^"
        ),
        (__Almanac["DAY_PHASE"] = "night"),
        assert_equal(
            testOb->query_localsky(__Almanac, "sky"),
            "%^BLUE%^BOLD%^The sky is darkened with night.%^RESET%^"
        ),
    }) :));
    ...
    expect("localsky describes the sun per day phase", (: ({
        (__Almanac["DAY_PHASE"] = "dawn"),
        assert_equal(
            testOb->query_localsky(__Almanac, "sun"),
            "%^ORANGE%^The sun is hanging low in the dawning eastern sky.%^RESET%^"
        ),
        (__Almanac["DAY_PHASE"] = "day"),
        assert_equal(
            testOb->query_localsky(__Almanac, "sun"),
            "%^I_YELLOW%^BOLD%^The sun is shining brightly in the daytime sky.%^RESET%^"
        ),
        (__Almanac["DAY_PHASE"] = "dusk"),
        assert_equal(
            testOb->query_localsky(__Almanac, "sun"),
            "%^CYAN%^BOLD%^The sun is falling into the twilight sky.%^RESET%^"
        ),
...
void test_astronomy_from_room() {
    ...
    expect("query_astronomy_from_room only resolves domain rooms", (: ({
        ...
        assert_equal(
            testOb->query_astronomy_from_room("/domain/Nowhere/room/x"),
            0
        ),
        // a room object whose path is not under /domain/
        assert_regex(
            file_name(roomOb = new(STD_ROOM)),
            STD_ROOM[0..<3] + "#[0-9]+"
        ),
    ...
    expect("query_astronomy_from_room resolves and caches a real domain file", (: ({
        ...
        assert_equal(
            mapp(testOb->query_astronomy_from_room("/domain/Start/human/room/x")),
            1
        ),
        // a second call with the cached file path hits the cache branch
        assert_equal(
            mapp(testOb->query_astronomy_from_room("/domain/Start/human/astronomy.c")),
            1
        ),
    }) :));
}
